...
class Game
{
    ...
    public function __construct(array $armies, string $logFile)
    {
    ...
    private function resetUnits(): void
    {
        foreach ($this->armies as $key => $army) {
            $this->armies[$key]->resetUnits($army->maxUnits);
        }
    }

    private function play(): void
    {
        $currentPlayer = $this->getRandArmy(); # pick random army
        $currentEnemy = $this->getRandArmy([$currentPlayer]); # pick random army exclude currentPlayer 
        $move_id = 1;

        # Игра продолжается, пока есть хотя бы две живые армии
        while ($this->hasTwoPlayers()) {

            if ($currentPlayer->canMove() and $currentEnemy->canMove()) {
                file_put_contents($this->logFile, "Ход $move_id: ", FILE_APPEND);

                $attacker = $currentPlayer->getActiveUnit();
                $target = $currentEnemy->getActiveUnit();
                $currentPlayer->makeMove($currentEnemy, $attacker, $target);

                # Месть. Смена ролей
                if ($target->active) {
                    $move_id++;
                    file_put_contents($this->logFile, "Ход $move_id: Ответная Атака! ", FILE_APPEND);
                    $currentEnemy->makeMove($currentPlayer, $target, $attacker);
                }
            } else {
                # Какая-то из армий выбыла, находим и удаляем её
                $loser = $currentPlayer->canMove() ? $currentEnemy : $currentPlayer;
        ...
        //* Логирование
        $gameResult = "Победила армия \"$winner->name\"\n";

        $dead = $winner->getDead();
        $deadCount = $winner->countDead();
        $gameResult .= "Выбыло: $deadCount ($dead)\n";

        $alive = $winner->getAlive();
        $aliveCount = $winner->countAlive();
     ...
     * @return Army
     */
    private function getRandArmy(array $exclude = []): Army
    {
        $armies = $this->armies;
        ...
        $randId = array_rand($armies);
        $r = $armies[$randId];

        return $r;
    }

    private function hasTwoPlayers(): bool
    {
    ...
    {
        file_put_contents(self::$logFile, $message, $mode);
    }
}
...
class Army
{
    ...
    public function __construct($name, int $maxUnits)
    {
    ...
    private function makeUnits($maxUnits)
    {
    ...
    public function makeMove(Army $enemy_army, Unit $attacker, Unit $target)
    {
        $attacker->attack($target);

        //* Логирование
        $this->attackLog($attacker, $target, $enemy_army);
    }


    private function attackLog(Unit $attacker, Unit $target, Army $enemy_army)
    {
    ...
    {
        $randId = array_rand($this->units);
        $rand_unit = $this->units[$randId];
    ...
    public function getUnitsHealth(): int
    {
        $res = 0;
        foreach ($this->units as $unit) {
...
class Unit
{
    ...
    {
        $this->health = self::MAX_UNIT_HEALTH;
        $this->damage = random_int(self::MIN_UNIT_DAMAGE, self::MAX_UNIT_DAMAGE);
    ...
    private function takeDamage($damage)
    {
        ...
        if ($this->health < self::MIN_UNIT_HEALTH) {
            $this->active = false;
            $this->destroyed = true;
        }
    }
